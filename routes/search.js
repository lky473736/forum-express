...
router.get('/:page', async (요청, 응답) => {
    ...
    try {
        let postlist = await db.collection('post').find({"title" : {'$regex' : 요청.query.keyword}}).skip((요청.params.page-1) * 5).limit(5).toArray();
        let allPostlist = await db.collection('post').find({"title" : {'$regex' : 요청.query.keyword}}).toArray();
